...
class EGCPPacket:
    ...
    SENSOR_MAP = {
        ...
        "P7": 0x07, "P8": 0x08,
        "T1": 0x07,  # thermocouple
        "L1": 0x08, "L2": 0x09, "L3": 0x0A,  # load cells
    }
...
class VTComms(QObject):
    ...
    def set_valve_failure(self, valve_id, enabled):
        ...
        if valve_id in self.valve_failures:
            ...
            self.log_signal.emit(f"{'⚠️ FAIL' if enabled else '✓ OK'}: {valve_name} failure mode {'ENABLED' if enabled else 'disabled'}")
    ...
    def send_sensors(self):
        ...
        # Send pressure sensors (P1-P6) - 3 per packet (5 bytes each = 15 bytes max)
        sensors_to_send = [
            ("P1", self.sensor_data.get("P1", 0.0)),
            ("P2", self.sensor_data.get("P2", 0.0)),
            ("P3", self.sensor_data.get("P3", 0.0)),
            ("P4", self.sensor_data.get("P4", 0.0)),
            ("P5", self.sensor_data.get("P5", 0.0)),
            ("P6", self.sensor_data.get("P6", 0.0)),
            ("P7", self.sensor_data.get("P7", 0.0)),
            ("P8", self.sensor_data.get("P8", 0.0)),
            ("T1", self.sensor_data.get("T1", 0.0)),
            ("L1", self.sensor_data.get("L1", 0.0)),
            ("L2", self.sensor_data.get("L2", 0.0)),
            ("L3", self.sensor_data.get("L3", 0.0)),
        ]
        
        # Send in groups of 3 (5 bytes per sensor = 15 bytes per packet)
        for i in range(0, len(sensors_to_send), 3):
            body = b''
            for sensor_name, sensor_value in sensors_to_send[i:i+3]:
