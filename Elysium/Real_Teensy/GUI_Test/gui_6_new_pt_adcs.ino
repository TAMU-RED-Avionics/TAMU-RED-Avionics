...
const long unsigned NOOP_RX_TIMEOUT =  100 * 1000;        // Timeout to consider a lack of a heartbeat as a miss (microsec) - 100ms (10x10ms)
...
// Valves
const int NCS1_PIN = 7;
const int NCS2_PIN = 8;
const int NCS3_PIN = 11;
const int NCS5_PIN = 0;
const int NCS6_PIN = 0;
const int LABV1_PIN = 5;
const int GV1_PIN = 0;
const int GV2_PIN = 0;
const int GIMBAL_ENABLE_PIN = 14;
...
void loop() {
  ...
  if ((micros() - LAST_SENSOR_UPDATE) > SENSOR_UPDATE_INTERVAL) {
    ...
    // Read from external ADCs via SPI
    // Channel 0 typically for PT1, Channel 1 for PT2 (Assumption based on typical ordering)
    float p1_psia = convert_PX2AF1XX500PAAAX_PT(read_adc(0));
    float p2_psig = convert_6017N19_PT(read_adc(1));
