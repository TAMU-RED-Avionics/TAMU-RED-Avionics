...
const unsigned long PRECISION = 5;
...
void loop() {
  ...
  // Sensor readings - send as ADC packet
  if ((micros() - LAST_SENSOR_UPDATE) > SENSOR_UPDATE_INTERVAL) {
    ...
    // Pack sensor data: [SENSOR_ID][4-byte float value] repeated
    uint8_t sensor_data[15];
    int data_idx = 0;
    
    // Add pressure sensor readings
    sensor_data[data_idx++] = EGCPPacket::SENSOR_P1;
    memcpy(&sensor_data[data_idx], &CUR_FAKE_VAL, sizeof(float));
    data_idx += 4;
    
    EGCPPacket adc_pkt(PACKET_ID_COUNTER++, EGCPPacket::PKT_ADC, 
                       sensor_data, data_idx);
    tx_egcp_packet(adc_pkt);
  }
}